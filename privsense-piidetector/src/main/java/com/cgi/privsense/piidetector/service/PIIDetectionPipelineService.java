...
public class PIIDetectionPipelineService {
    ...
    public PIIDetectionPipelineService(
            ...
            @Value("${privsense.piiDetector.confidence.threshold}") double confidenceThreshold) {
    ...
    public ColumnPIIInfo analyzeColumn(String connectionId, String dbType, String tableName,
        ...
        // Validate table and column names using common DatabaseUtils
        try {
            DatabaseUtils.validateTableName(tableName);
            DatabaseUtils.validateColumnName(columnName);
        } catch (IllegalArgumentException e) {
            log.warn("Invalid table or column name: {}.{} - {}", tableName, columnName, e.getMessage());
        ...
        if (heuristicStrategy.isApplicable(true, false)) {
            ...
            try {
                ...
                throw new PIIDetectionException("Error during heuristic detection: " + e.getMessage(), e);
            }
        ...
        if (regexStrategy.isApplicable(true, true) && !filteredSamples.isEmpty()) {
            ...
            try {
                ...
                log.error("Error during regex detection on {}.{}: {}",
                        tableName, columnName, e.getMessage(), e);
                throw new PIIDetectionException("Error during regex detection: " + e.getMessage(), e, "REGEX_ERROR");
            }
        ...
        // Stage 3: NER detection (more expensive, applied as a last resort)
        {
            ...
            try {
                ...
                log.error("Error during NER detection on {}.{}: {}",
                        tableName, columnName, e.getMessage(), e);
                throw PIIDetectionException.serviceError("NER service error: " + e.getMessage());
            }
