...
/**
 * Implementation of SamplingStrategy that optimizes for parallel execution of
 * sampling operations.
 */
@Slf4j
@Service
public class OptimizedParallelSamplingStrategy implements SamplingStrategy, DisposableBean {
    ...
    public OptimizedParallelSamplingStrategy(
        ...
        // Create execution service with configuration from database properties
        this.executionService = new ParallelExecutionService(
                databaseProperties.getTasks().getThreadPoolSize(),
                databaseProperties.getSampling().getTimeout(),
                databaseProperties.getSampling().getTimeoutUnit());
